...
def user_may_run_in_district(
    ...
    """
    Czy osoba z danej jednostki terytorialnej może kandydować / być w rankingu.

    Kotwicą może być obwód, gmina, powiat itd. (np. gmina urodzenia
    zamiast komisji). Poziom zgodności: `office.candidacy_level`.
    """
    if unit is None:
        return False
    ...
    if level is None:
        ...
        if not district_unit_ids:
            return False
        return unit_is_descendant_of_any(unit, district_unit_ids)
